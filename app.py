...
TRANSACTION_KEYWORDS = {
    "BUY": ["purchase", "buy", "sip", "subscription", "switch in", "switch-in", "allotment"],
    "SELL": ["sell", "redemption", "redeem", "switch out", "switch-out"],
    "DIVIDEND": ["dividend payout", "dividend", "idcw payout", "income distribution"],
    "DIV_REINVEST": ["dividend reinvest", "idcw reinvest"],
    "BONUS": ["bonus"],
    "RIGHTS": ["rights"],
}
...
def classify_transaction_line(line: str) -> str | None:
    ll = line.lower()
    # More specific phrases must be checked before generic dividend.
...
def infer_transaction_numbers(line: str, date_token: str) -> tuple[Decimal | None, Decimal | None]:
    """
    Conservative fallback: quantity is usually the penultimate numeric field and
    amount/value the last numeric field. Returns (quantity, amount).
    """
    cleaned = line.replace(date_token, " ")
...
def parse_cas_pdf(
    ...
    for line in raw_lines:
        ...
        if dm and txn_type and current_isin:
                ...
                transactions.append(
                    {
                        ...
                        "amount_inferred": None if amount is None else float(abs(amount)),
                        "cashflow_inferred": None if signed_amount is None else float(signed_amount),
                        "transaction_confidence": 0.55,
                        "source_line": line[:700],
                    }
                )
...
if section == "Home":
    ...
    coverage = pd.DataFrame(
        [
            ...
            ["Multi-CAS reconstruction", "Partial", "Holdings + heuristic dated transaction reconstruction"],
...
# -----------------------------------------------------------------------------
    ...
    status = pd.DataFrame(
        [
            ...
            ["Automated CAS XIRR", "Implemented starter", APP_VERSION],
