...
# Streamlit's cache decorators inspect function source while this generated core is
# being executed. Under Python 3.14, compiling the transformed source with the
# original file name but leaving linecache pointed at the unmodified disk file can
# make inspect/tokenize start at the wrong line and raise TokenError. Register the
# exact generated source under the same filename before exec so inspect.getsource()
# always sees code that matches the compiled line numbers.
runtime_filename = str(CORE_PATH)
linecache.cache[runtime_filename] = (
