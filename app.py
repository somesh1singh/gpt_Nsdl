...
def find_sgb_values(
    ...
    """
    SGB layout after maturity date:
    units, face-value/unit, market-price/unit, market-value.
    Validation uses units × market-price ≈ market-value.
    """
    maturity_idx = next(
...
def parse_cas_pdf(
    ...
    for line in raw_lines:
        ...
        if m:
            isin = m.group(0)
            current_isin = isin
            current_name = holding_name_map.get(isin, isin)
...
# -----------------------------------------------------------------------------
    ...
    with auto_tab:
        ...
        c2.metric("Latest CAS terminal value", fmt_inr(terminal_value))
        avg_conf = (
