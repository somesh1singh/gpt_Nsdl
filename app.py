...
from historical_holdings_bridge import (
    apply_historical_holdings_bridge,
    plan_historical_holdings_dates,
)
...
# validate the same monetary/XIRR contracts after the v1.1.22 integration layer.
...
def xirr_diagnostic_workbook(result: dict[str, Any], readiness: dict[str, Any]) -> bytes:
    ...
    sheets = {
        ...
        "Historical_Holdings_Evidence": result.get("historical_holdings_evidence", pd.DataFrame()),
...
HISTORICAL_HOLDINGS_PLUS_POST_SNAPSHOT_TRADES
'''

# -----------------------------------------------------------------------------
# v1.1.22 runtime integration
# -----------------------------------------------------------------------------
...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"v1.1.22 integration anchor {label!r} expected once, found {count}")
    return text.replace(old, new, 1)


source = replace_once(source, "# APP VERSION: 1.1.15", "# APP VERSION: 1.1.22", "header version")
source = replace_once(source, 'APP_VERSION = "1.1.15"', 'APP_VERSION = "1.1.22"', "runtime version")
...
result_replacement = '            "reconciliation": pd.DataFrame(reconciliation), "resolution_candidates": pd.DataFrame(resolution_candidates), "resolution_ledger": pd.DataFrame(resolution_ledger), "corporate_action_evidence": pd.DataFrame(corporate_action_evidence), "cas_bridge_evidence": pd.DataFrame(), "historical_holdings_evidence": pd.DataFrame(), "account_ids": sorted(account_ids), "ledger": pd.DataFrame(ledger),\n'
...
source = replace_once(source, sheet_anchor, sheet_replacement, "diagnostic evidence sheets")
...
ui_replacement = '''    for key, label in [("reconciliation", "Quantity reconciliation"), ("resolution_ledger", "Applied audited resolution ledger"), ("cas_bridge_evidence", "CAS snapshot bridge evidence"), ("historical_holdings_evidence", "Historical holdings bridge evidence"), ("corporate_action_evidence", "Official NSE corporate-action evidence"), ("resolution_candidates", "Resolution candidates"), ("rejected", "Rejected/duplicate row audit"), ("identifier_review", "Missing ISIN evidence review"),\n'''
